...
if [[ ! -d logs/kmc-genomescope_np || ! -d logs/slurm_jobid ]]; then
    mkdir -p logs/kmc-genomescope_np
    mkdir -p logs/slurm_jobid
    echo -e "JobName,JobID,SubmissionTime" > logs/slurm_jobid/kmc-genomescope_np_slurm_jobid.csv
else
    cp logs/slurm_jobid/kmc-genomescope_np_slurm_jobid.csv logs/slurm_jobid/kmc-genomescope_np_slurm_jobid.csv.bak
fi

for SAMPLE in "${SAMPLES_FULL[@]}"; do

    # Set the output and input
    OUTPUT_DIR="${ROOTDIR}/results/kmc-genomescope_np/${SAMPLE}"
    FASTQ_IN="${ROOTDIR}/data/subsampled/${SAMPLE}/${SAMPLE}_OG.nanopore.fq"
    ...
    if [[ -s "${OUTPUT_DIR}/summary.txt" ]]; then
        ...
        if squeue --me --format "%.100j" | grep -q "kmc-genomescope_np_${SAMPLE}"; then
            ...
            sed -i "/kmc-genomescope_np_${SAMPLE}/d" logs/slurm_jobid/kmc-genomescope_np_slurm_jobid.csv
            mkdir -p ${OUTPUT_DIR}
        fi
    fi

sbatch <<- EOF | sed -e "s/Submitted batch job /kmc-genomescope_np_${SAMPLE},/" -e "s/$/,$(date +'%Y-%m-%d %H:%M:%S')/" >> logs/slurm_jobid/kmc-genomescope_np_slurm_jobid.csv
#!/bin/env bash

#SBATCH --job-name=kmc-genomescope_np_${SAMPLE}
#SBATCH --output=logs/kmc-genomescope_np/kmc-genomescope_np_${SAMPLE}.log
#SBATCH --ntasks=10
#SBATCH --time=1-00
#SBATCH --mem=50G
