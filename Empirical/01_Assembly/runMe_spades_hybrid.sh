    ...
    for DEPTH_NP in "${depthX_NP[@]}"; do
        for DEPTH_IL in "${depthX_IL[@]}"; do
            ...
            if [[ ! -s ${ILLUMINA1_FQ} || ! -s ${ILLUMINA2_FQ} || ! -s ${NANOPORE_FQ} ]]; then
                ...
                continue
            elif [[ -s "${ROOTDIR}/results/assemblies_results/${SAMPLE}/spades_hybrid_${SAMPLE}_NP${DEPTH_NP}_IL${DEPTH_IL}.fasta" || -s "${ROOTDIR}/results/assemblies_results/${SAMPLE}/spades_hybrid_${SAMPLE}_NP${DEPTH_NP}_IL${DEPTH_IL}.fasta.gz" ]]; then
                echo_overwrite "SPADES hybrid for ${SAMPLE} at NP${DEPTH_NP}_IL${DEPTH_IL} already completed. Skipping."
                continue
            else
...
#SBATCH --output=logs/spades_hybrid/spades_hybrid_${SAMPLE}_NP${DEPTH_NP}_IL${DEPTH_IL}.out
#SBATCH --time=1-00
#SBATCH --ntasks=10
#SBATCH --mem=100GB
