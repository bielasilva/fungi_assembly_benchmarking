...
for SAMPLE in "${SAMPLES[@]}"; do
    ...
    for DEPTH_IL in "${depthX_IL[@]}"; do
        ...
        if [[ ! -s ${ILLUMINA1_FQ} || ! -s ${ILLUMINA2_FQ} ]]; then
            ...
            continue
        elif [[ -s "${OUT_DIR}/spades_short_${SAMPLE}.fasta" ]]; then
            continue
        elif [[ -s "${ROOTDIR}/results/assemblies_results/${SAMPLE}/spades_short_${SAMPLE}_IL${DEPTH_IL}.fasta" || -s "${ROOTDIR}/results/assemblies_results/${SAMPLE}/spades_short_${SAMPLE}_IL${DEPTH_IL}.fasta.gz" ]]; then
            continue
        else
...
#SBATCH --output=logs/spades_short/spades_short_${SAMPLE}_IL${DEPTH_IL}.out
#SBATCH --time=1-00
#SBATCH --ntasks=10
#SBATCH --mem=180GB
