...
source /home/gas0042/miniforge3/bin/activate Polypolish
    ...
    for DEPTH_NP in "${depthX_NP[@]}"; do
        for DEPTH_IL in "${depthX_IL[@]}"; do
            ...
            if [[ ! -s "${FLYE_ASM}" ]]; then
                ...
                continue
            elif [[ -s "${ROOTDIR}/results/assemblies_results/${SAMPLE}/polypolish_${SAMPLE}_NP${DEPTH_NP}_IL${DEPTH_IL}.fasta" ]]; then
                # echo_overwrite "Polypolish for ${SAMPLE} at NP${DEPTH_NP}_IL${DEPTH_IL} is already completed. Skipping."
                continue
            else
...
#SBATCH --output=logs/polypolish/polypolish_${SAMPLE}_NP${DEPTH_NP}_IL${DEPTH_IL}_%j.out
#SBATCH --time=5-00
#SBATCH --ntasks=10
#SBATCH --mem=180GB
