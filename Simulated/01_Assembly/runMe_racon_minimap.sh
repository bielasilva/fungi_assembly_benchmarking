...
if [[ ! -d logs/racon-mp2 || ! -d logs/slurm_jobid ]]; then
    mkdir -p logs/racon-mp2
    mkdir -p logs/slurm_jobid
    echo -e "JobName,JobID,SubmissionTime" > logs/slurm_jobid/racon-mp2_slurm_jobid.csv
else
    cp logs/slurm_jobid/racon-mp2_slurm_jobid.csv logs/slurm_jobid/racon-mp2_slurm_jobid.csv.bak
fi
...
for SAMPLE in "${SAMPLES[@]}"; do
    ...
    for DEPTH_NP in "${depthX_NP[@]}"; do
        for DEPTH_IL in "${depthX_IL[@]}"; do
            ...
            OUT_DIR="${ROOTDIR}/results/assemblies/${SAMPLE}/racon-mp2/NP${DEPTH_NP}_IL${DEPTH_IL}"

            MINIMAP2_SAM="${OUT_DIR}/${SAMPLE}_NP${DEPTH_NP}_IL${DEPTH_IL}.sorted.sam"
            RACON_OUT="${OUT_DIR}/racon-mp2_${SAMPLE}_NP${DEPTH_NP}_IL${DEPTH_IL}"
            ...
            if [[ ! -s "${FLYE_ASM}" ]]; then
                ...
                continue
            elif [[ -s "${ROOTDIR}/results/assemblies_results/${SAMPLE}/racon-mp2_${SAMPLE}_NP${DEPTH_NP}_IL${DEPTH_IL}.fasta" ]]; then
                # echo_overwrite "Racon for ${SAMPLE} at NP${DEPTH_NP}_IL${DEPTH_IL} is already completed. Skipping."
                continue
            else
                if [[ $(squeue --me | wc -l) -ge 5000 ]]; then
                    ...
                    exit 1
                elif squeue --me --format "%.100j" | grep -q "racon-mp2_${SAMPLE}_NP${DEPTH_NP}_IL${DEPTH_IL}"; then
                    continue
                else
                    new_jobs=$((new_jobs + 1))
                    sed -i "/racon-mp2_${SAMPLE}_NP${DEPTH_NP}_IL${DEPTH_IL}/d" logs/slurm_jobid/racon-mp2_slurm_jobid.csv
                    rm -rf ${OUT_DIR}
                fi
            fi
...
sbatch <<- EOF | sed -e "s/Submitted batch job /racon-mp2_${SAMPLE}_NP${DEPTH_NP}_IL${DEPTH_IL},/g" -e "s/$/,$(date +'%Y-%m-%d %H:%M:%S')/" >> logs/slurm_jobid/racon-mp2_slurm_jobid.csv
#!/bin/env bash

#SBATCH --job-name=racon-mp2_${SAMPLE}_NP${DEPTH_NP}_IL${DEPTH_IL}
#SBATCH --output=logs/racon-mp2/racon-mp2_${SAMPLE}_NP${DEPTH_NP}_IL${DEPTH_IL}.out
#SBATCH --time=0-10
#SBATCH --ntasks=10
#SBATCH --mem=100GB
...
# Run minimap2 to align Illumina reads to the FLYE assembly
minimap2 \
    -a -t 10 \
    ${FLYE_ASM} \
    ${SAMPLE}_${DEPTH_IL}.combined.fq > ${MINIMAP2_SAM}
...
if [[ ! -s ${RACON_OUT}.fasta ]]; then
    echo "Racon failed for ${SAMPLE} at NP${DEPTH_NP}_IL${DEPTH_IL}. Exiting."
    exit 1
fi
EOF
        done
