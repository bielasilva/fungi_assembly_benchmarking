...
# 02_fit_models.R
# ----------------------------------------------------------------------------
# Fit linear mixed-effects models for simulated data with response transforms,
# DHARMa residual diagnostics, and exported coefficient tables.
#
# Sources:
#   - 01_data_prep.R (must be sourced first to populate `data_simulated`)
#
# Outputs (under Simulated/R/results/):
...
dir.create(file.path(results_dir, "individual_files"), showWarnings = FALSE, recursive = TRUE)
# ----------------------------------------------------------------------------
...
response_transforms <- c(
  ...
  busco_complete_diff     = "none",
  genome_fraction_percent = "logit",
  length_err_pct          = "none",
  mism_100kbp             = "log1p",
  indel_100kbp            = "log1p",
  maxrss_gb               = "log10",
...
# depth variable and size_mbp as the genome covariate.
fit_block_single <- function(df, response, transform_type) {
  ...
  rhs_single <- "program * depth_eff + (size_mbp_s + gc_content_s + gene_density_s + cds_per_gene_s) +
                 program:(size_mbp_s + gc_content_s + gene_density_s + cds_per_gene_s) +
                 depth_eff:(size_mbp_s + gc_content_s + gene_density_s + cds_per_gene_s) +
                 (1 | sample)"
...
fit_block_multi <- function(df, response, transform_type) {
  ...
  rhs_multi <- "program * depth_np * depth_il + (size_mbp_s + gc_content_s + gene_density_s + cds_per_gene_s) +
                 program:(size_mbp_s + gc_content_s + gene_density_s + cds_per_gene_s) +
                 depth_np:(size_mbp_s + gc_content_s + gene_density_s + cds_per_gene_s) +
                 depth_il:(size_mbp_s + gc_content_s + gene_density_s + cds_per_gene_s) +
                 (1 | sample)"
...
extract_genome_slopes <- function(fit) {
  covars <- c("size_mbp_s", "gc_content_s", "gene_density_s", "cds_per_gene_s")
  purrr::map_dfr(covars, function(v) {
    emmeans::emtrends(fit$model, ~ program, var = v, pbkrtest.limit = 70000) %>%
      summary(infer = TRUE) %>%
      as_tibble() %>%
      rename(slope := !!paste0(v, ".trend"), t_ratio = t.ratio, p_value = p.value) %>%
      mutate(
        signif    = sig_stars(p_value),
        covariate = v,
        response  = fit$response,
...
fit_and_export_single <- function(df, response, transform_type, model_label) {
  ...
  # Save tables
  write_csv(global_effects, file.path(results_dir, "individual_files", paste0(model_label, "_", response, "_global_effects.csv")))
  write_csv(fixed_effects,  file.path(results_dir, "individual_files", paste0(model_label, "_", response, "_fixed_effects.csv")))
  write_csv(depth_slopes,   file.path(results_dir, "individual_files", paste0(model_label, "_", response, "_depth_slopes.csv")))
  write_csv(genome_slopes,  file.path(results_dir, "individual_files", paste0(model_label, "_", response, "_genome_slopes.csv")))
  write_csv(prog_pairs,     file.path(results_dir, "individual_files", paste0(model_label, "_", response, "_prog_pairs.csv")))
  write_csv(depth_pairs,    file.path(results_dir, "individual_files", paste0(model_label, "_", response, "_depth_pairs.csv")))
  write_csv(dharma_summary, file.path(results_dir, "individual_files", paste0(model_label, "_", response, "_dharma_summary.csv")))
...
if (!exists("data_simulated")) {
  stop("data_simulated not found. Source 01_data_prep.R first.")
}

data_single <- data_simulated %>% 
  filter(strategy %notin% c("hybrid", "polished")) %>% 
  left_join(reference_metrics, by="sample")

data_multi  <- data_simulated %>%
...
saveRDS(single_fits, file.path(results_dir, "simulated_single_fits_list.rds"))

# --- Single-strategy fits No Miniasm ---
single_fits_no_miniasm <- imap(response_transforms, in_parallel(function(transform_type, response) {
  data_single_no_miniasm <- data_single %>% filter(program != "miniasm")
  fit_and_export_single(
    df             = data_single_no_miniasm,
    response       = response,
    transform_type = transform_type,
    model_label    = "simulated_single_no_miniasm"
  )
}, fit_and_export_single=fit_and_export_single,fit_block_single=fit_block_single,
data_single=data_single, apply_transform=apply_transform, mk_form=mk_form,
extract_global_effects=extract_global_effects, sig_stars=sig_stars,
...
single_fits_no_miniasm <- compact(single_fits_no_miniasm)
...
for (strat in c("short_reads", "long_reads")) {
  ...
  fits_this_strat <- imap(response_transforms, in_parallel(function(transform_type, response) {
    ...
    fit_and_export_single(
      ...
      response       = response,
      transform_type = transform_type,
      model_label    = paste0("simulated_single_", strat)
    )
  }, fit_and_export_single=fit_and_export_single,fit_block_single=fit_block_single,
...
message("Simulated model fitting for single-strategy done.")
save.image(file.path(results_dir, "02_fit_models_single.RData"))
...
# --- Multi-strategy fits for each Seq Strategy ---
for (strat in c("polished", "hybrid")) {
  if (strat == "polished" ){
    data_strat <- data_multi %>% filter(strategy == "polished") %>% select(where(~ !all(is.na(.))))
  } else {
    data_strat <- data_multi %>% 
      filter(strategy == strat | program == "flye_polypolish")
  }
  ...
  fits_this_strat <- imap(response_transforms, in_parallel(function(transform_type, response) {
    ...
    fit_and_export_multi(
      ...
      response       = response,
      transform_type = transform_type,
      model_label    = paste0("simulated_multi_", strat)
    )
  }, fit_and_export_multi=fit_and_export_multi,fit_block_multi=fit_block_multi,
  data_strat=data_strat, apply_transform=apply_transform, mk_form=mk_form,
  extract_global_effects=extract_global_effects, sig_stars=sig_stars,
...
message("Simulated model fitting for multi-strategy done.")
save.image(file.path(results_dir, "02_fit_models_multi.RData"))
saveRDS(multi_fits, file.path(results_dir, "simulated_multi_fits_list.rds"))
saveRDS(within_strategy_fits, file.path(results_dir, "simulated_within_strategy_fits_list.rds"))
...
all_fixed_effects <- bind_rows(
  ...
  map_dfr(single_fits_no_miniasm, ~ .x$fixed_effects %>% mutate(model_set = "single_no_miniasm")),
...
write_csv(all_fixed_effects, file.path(results_dir, "simulated_all_fixed_effects.csv"))

all_global_effects <- bind_rows(
  map_dfr(single_fits, ~ .x$global_effects %>% mutate(model_set = "single")),
  map_dfr(single_fits_no_miniasm, ~ .x$global_effects %>% mutate(model_set = "single_no_miniasm")),
...
write_csv(all_global_effects, file.path(results_dir, "simulated_all_global_effects.csv"))

all_genome_slopes <- bind_rows(
  map_dfr(single_fits, ~ .x$genome_slopes %>% mutate(model_set = "single")),
  map_dfr(single_fits_no_miniasm, ~ .x$genome_slopes %>% mutate(model_set = "single_no_miniasm")),
...
write_csv(all_genome_slopes, file.path(results_dir, "simulated_all_genome_slopes.csv"))

all_slopes_effects <- bind_rows(
  map_dfr(single_fits, ~ .x$depth_slopes %>% mutate(model_set = "single", response = .x$fit$response)),
  map_dfr(single_fits_no_miniasm, ~ .x$depth_slopes %>% mutate(model_set = "single_no_miniasm", response = .x$fit$response)),
...
write_csv(all_slopes_effects, file.path(results_dir, "simulated_all_slopes_effects.csv"))
...
all_dharma <- bind_rows(
  ...
  map_dfr(single_fits_no_miniasm, ~ .x$dharma_summary %>% mutate(model_set = "single_no_miniasm")),
...
write_csv(all_dharma, file.path(results_dir, "simulated_all_dharma_diagnostics.csv"))
...
model_summary_table <- bind_rows(
  ...
  map_dfr(single_fits_no_miniasm, function(f) {
    tibble(
      model_set = f$model_label,
      response  = f$fit$response,
      transform = f$fit$transform,
...
write_csv(model_summary_table, file.path(results_dir, "simulated_model_summary_table.csv"))

message("Simulated model fitting complete. ", nrow(model_summary_table), " models fit.")
