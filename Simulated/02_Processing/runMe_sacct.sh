...
JOBID_FILES=(
        ...
        "${SLURM_DIR}/abyss-short_slurm_jobid.csv"
...
for JOBID_FILE in "${JOBID_FILES[@]}"; do
    ...
    sacct --format=JobID,JobName%100,Start,End,Elapsed,ReqMem,MaxRSS,ReqCPUS,NCPUS,CPUTime,CPUTimeRAW,Partition,ExitCode -P --delimiter="," -j $JOBID_LIST > ${JOBID_FILE%_jobid.csv}_sacct.csv
done

echo "JobID,JobName,Start,End,Elapsed,ReqMem,MaxRSS,ReqCPUS,NCPUS,CPUTime,CPUTimeRAW,Partition,ExitCode" > ${SLURM_DIR}/all_sacct.csv

cat ${SLURM_DIR}/*slurm_sacct.csv | sed "/JobID,JobName,Start,End,Elapsed,ReqMem,MaxRSS,ReqCPUS,NCPUS,CPUTime,CPUTimeRAW,Partition,ExitCode/d" >> ${SLURM_DIR}/all_sacct.csv